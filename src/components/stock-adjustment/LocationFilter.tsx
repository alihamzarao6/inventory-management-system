...
const LocationFilter: React.FC<LocationFilterProps> = ({
  ...
  // Set the main location if a sub-location is selected
  useEffect(() => {
    if (selectedLocationId && selectedLocationId.startsWith("sub-")) {
      const subLocation = MOCK_SUB_LOCATIONS.find(
        (sl) => sl.id === selectedLocationId
      );
      if (subLocation) {
        setSelectedMainLocation(subLocation.parentId);
      }
    } else if (selectedLocationId) {
      setSelectedMainLocation(selectedLocationId);
    }
  }, [selectedLocationId]);
  ...
  const handleMainLocationSelect = (locationId: string) => {
    ...
    // If we're switching to a different main location, clear the selection
    if (
      selectedLocationId &&
      selectedLocationId !== locationId &&
      (!selectedLocationId.startsWith("sub-") ||
        !MOCK_SUB_LOCATIONS.find(
          (sl) => sl.id === selectedLocationId && sl.parentId === locationId
        ))
    ) {
      if (onLocationSelect) {
        onLocationSelect("");
                ...
                <>
                  ...
                  <div
                    className={cn(
                      ...
                      selectedLocationId === selectedMainLocation &&
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          disabledLocationIds.includes(subLoc.id)
                            ? "opacity-50 cursor-not-allowed"
                            : "hover:bg-gray-100 cursor-pointer",
                          selectedLocationId === subLoc.id && "bg-blue-50"
                        )}
                        onClick={() =>
                          !disabledLocationIds.includes(subLoc.id) &&
