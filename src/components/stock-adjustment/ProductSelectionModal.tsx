...
interface ProductSelectionModalProps {
  ...
  selectedLocationIds: string[]; // Changed from selectedLocationId to selectedLocationIds
  availableProducts: Product[];
  existingItems: StockAdjustmentItem[];
  onAddNewProduct?: () => void; // Optional callback for "Add New Product" button
}
        ...
        <DialogHeader className="bg-white px-6 py-4 mt-3 border-b shrink-0">
          <div className="flex justify-between items-center">
            <DialogTitle className="text-xl font-semibold">
              Select Products for Stock Adjustment
            </DialogTitle>
            {onAddNewProduct && (
              <Button
                variant="outline"
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map((product, index) => {
                      ...
                      // Calculate current quantity from selected location
                      const currentQuantity = getItemQuantity(product);
